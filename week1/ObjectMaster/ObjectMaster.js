...
const pkmnMore1 = pokémon.filter(p => p.types.length>1)
...
const pkmnNamesPoison = pokémon.filter( p => p.types.length ==1 && p.types[0]==="poison" ).map(p => p.name)
...
console.table(pkmnNames)
console.table(pkmnNames99)
console.table(pkmnNamesPoison)
console.table(pkmnFlying)
console.table(normalCount)
